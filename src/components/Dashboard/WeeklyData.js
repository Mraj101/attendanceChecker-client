...
const WeeklyData = ({ weeklysummaryData, loading, date }) => {
  const [dateLen, setDateLen] = useState(0);
  const [newDateList, setNewDateList] = useState([]);

  useEffect(() => {
    if (weeklysummaryData.length > 0) {
      if (weeklysummaryData[0].dateList) {
        let newDateLen = weeklysummaryData[0].dateList.length;
        setDateLen(newDateLen);
        let list = [];
        for (let i = 0; i < newDateLen; i++) {
          list.push(weeklysummaryData[0].dateList[i].date);
          setNewDateList(list);
        }
      }
    }
  }, [weeklysummaryData, loading]);
